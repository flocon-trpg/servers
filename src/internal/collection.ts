...
class $Iterator<T> {
    ...
    public compact<TResult>(
        mapping: (element: T, index: number) => TResult | null | undefined
    ): $Iterator<Exclude<TResult, null | undefined>> {
    ...
    public find(
        predicate: (elem: T, index: number) => boolean
    ...
    public flatMap<TResult>(
        mapping: (element: T, index: number) => TResult[]
    ): $Iterator<TResult> {
        const baseIterate = this.iterate;
        function* iterate() {
            let index = 0;
    ...
    public mapAsync<TResult>(
        mapping: (element: T, index: number) => PromiseLike<TResult>
    ) {
        return this.toAsync(this.iterate).mapAsync(mapping);
    }

    public pairwise(): $Iterator<{
        prev: T | undefined;
        current: T;
        next: T | undefined;
    }> {
    ...
    public reduce<TResult>(
        mapping: (seed: TResult, element: T, index: number) => TResult,
        seed: TResult
    ): TResult {
        let result = seed;
        let index = 0;
    ...
    public toMap<TKey, TValue>(
        mapping: (source: T) => { key: TKey; value: TValue }
    ): Map<TKey, TValue> {
    ...
    public toMapAsync<TKey, TValue>(
        mapping: (source: T) => Promise<{ key: TKey; value: TValue }>
    ) {
...
class $AsyncIterator<T> {
    public constructor(
        private readonly iterateAsync: () => AsyncIterableIterator<T>
    ) {}
    ...
    ): $AsyncIterator<Exclude<TResult, null | undefined>> {
    ...
    public async reduceAsync<TResult>(
        mapping: (
            seed: TResult,
            element: T,
            index: number
    ...
    public async toMapAsync<TKey, TValue>(
        mapping: (source: T) => PromiseLike<{ key: TKey; value: TValue }>
...
class $ReadonlyArray<T> extends $Iterator<T> {
    ...
    public async findOrUndefinedAsync(
        predicateAsync: (element: T) => Promise<boolean>
    ): Promise<T | undefined> {
...
class $ReadonlySet<T> extends $Iterator<T> {
    ...
    public groupJoin(right: ReadonlySet<T> | $ReadonlySet<T>) {
        const rightSource =
            right instanceof $ReadonlySet ? right.source : right;
...
class $ReadonlyMap<TKey, TValue> extends $Iterator<[TKey, TValue]> {
    ...
    public mapToArray<TResult>(
        mapping: (params: { key: TKey; value: TValue }) => TResult
    ): TResult[] {
    ...
    public forAll(
        predicate: (params: { key: TKey; value: TValue }) => boolean
    ): boolean {
...
*/
export function __<TValue>(
    source: ReadonlyArray<TValue> | Array<TValue> | $ReadonlyArray<TValue>
): $ReadonlyArray<TValue>;
export function __<TValue>(
    source: ReadonlySet<TValue> | Set<TValue> | $ReadonlySet<TValue>
): $ReadonlySet<TValue>;
export function __<TKey, TValue>(
    source:
