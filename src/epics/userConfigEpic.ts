...
export const userConfigEpic = (action$: ActionsObservable<AnyAction>, state$: StateObservable<RootState>): Observable<AnyAction> => {
    return new Observable<AnyAction>(observer => {
        return action$.pipe(
            Rx.map(action => {
                if (
                    userConfigModule.actions.set.match(action)
                ) {
                    return action;
                }
                return null;
