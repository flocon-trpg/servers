...
import { PongDocument, PongSubscription, PongSubscriptionVariables, usePingMutation, usePongSubscription } from '../../generated/graphql';
...
const PingPongCore: React.FC = () => {
    ...
    const [pongObservableResult, setPongObservableResult] = React.useState<PongObservableResultState>();

    React.useEffect(() => {
        const subscription = apolloClient.subscribe<PongSubscription, PongSubscriptionVariables>({ query: PongDocument }).subscribe(pong => {
            if (pong.data != null) {
    ...
    const pongObservableResponse: JSX.Element | JSX.Element[] | null = (() => {
        ...
        switch (pongObservableResult.type) {
            ...
            case 'errors':
                return pongObservableResult.errors.map((error, index) => (<div key={index}>{error.message}</div>));
    ...
    return (
        <div>
            ...
            <InputNumber value={postValue} onChange={value => typeof value === 'number' ? setPostValue(value) : undefined} />
            <Button onClick={() => pingMutation({ variables: { value: postValue } })} type="primary">Ping</Button>
