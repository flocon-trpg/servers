    ...
    return (
        <Layout requiresLogin={false} showEntryForm={false}>
            <div css={$css}>
                ...
                <p>JSONという単語を聞いたことがある方へ: TOMLとは、正確ではないですが大雑把に言うとJSONを簡単に記述するための言語です。JSONは広く使われている言語ですが、人間が手作業で書くには面倒な部分もあります。そこで、TOMLで記述したものをJSONに変換し、そのJSONをfloconが解釈するという流れにすることで、JSONを直接記述するより楽に書くことができます(実際は、floconはTOMLをJSONに変換せずにそのまま直接解釈します。JSONに関するこの解説は、あくまでイメージです)。ですので、TOMLはJSONと似ているというイメージを片隅に置いておくともしかしたら理解しやすいかもしれません。</p>
...
"名前" = "山田"`}
                ...
                <p><strong>2021/06/23に大きな変更がありました。</strong></p>

                <p>現在、キャラクターに対するコマンドのみがとりあえず実装されています。</p>

                <p>例えば、コマンドの名前は「名称変更」で、キャラの名前を「山田」に変更するコマンドを定義するには下のようになります。</p>
...
name = "山田"`}
                ...
                <p>[[_]] は、1つのコマンドの記述開始のような意味を表す記号です。_ はコマンドのルートを表す記号のようなものです。self は自分自身という感じの意味であり、ここでは選択されたキャラクターを表します。つまり、「選択されたキャラクターの名前を 山田 にする」といった感じの意味になります。</p>
...
name = "Yamada"`}
                ...
                <p>現状ではコマンドでできることは非常に限られており、名前と画像の変更とBGM再生/変更とSE再生しかできません(画像の変更を表すコマンドの設定方法は後々追加する予定です)。後々追加していく予定です。</p>
