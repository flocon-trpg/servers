...
export const recordDownOperationElementFactory = <TState extends t.Mixed, TOperation extends t.Mixed>(state: TState, operation: TOperation) => t.union([
    t.type({
...
export const recordUpOperationElementFactory = <TState extends t.Mixed, TOperation extends t.Mixed>(state: TState, operation: TOperation) => t.union([
    t.type({
        type: t.literal(replace),
        replace: t.partial({
            newValue: state,
...
export const mapRecordOperationElement = <TReplace1, TReplace2, TOperation1, TOperation2>({ source, mapOperation, mapReplace }: {
    source: {
        type: typeof replace;
        replace: TReplace1;
    } | {
