...
export const compose = <TOperation, TCustomError = string>({
    ...
    innerCompose: (params: { key: string; first: TOperation; second: TOperation }) => CustomResult<TOperation | undefined, string | TCustomError>;
...
export const serverTransform = <TServerState, TFirstOperation, TSecondOperation, TCustomError = string>({
    first,
    second,
    prevState,
    nextState,
...
type InnerClientTransform<TOperation, TError = string> = (params: {
    first: TOperation;
    second: TOperation;
}) => CustomResult<{ firstPrime: TOperation | undefined; secondPrime: TOperation | undefined }, TError>;
...
export const clientTransform = <TOperation, TError = string>({
...
}): CustomResult<{ firstPrime: Record<string, TOperation> | undefined; secondPrime: Record<string, TOperation> | undefined }, TError> => {
    ...
    groupJoin(recordToMap(first), recordToMap(second)).forEach((group, key) => {
        if (error != null) {
            return;
        }
        switch (group.type) {
...
export const diff = <TState, TOperation>({
    ...
    prevState: Record<string, TState>;
    nextState: Record<string, TState>;
    innerDiff: (params: { prevState: TState | undefined; nextState: TState | undefined; key: string }) => TOperation | undefined;
}): Record<string, TOperation> => {
    const result: Record<string, TOperation> = {};
    for (const [key, value] of groupJoin(recordToMap(prevState), recordToMap(nextState))) {
