...
export class CustomDualKeyMap<TKey, TKeySource1, TKeySource2, TValue> {
    ...
    public constructor(private readonly params: {
        sourceMap?: DualKeyMapSource<TKeySource1, TKeySource2, TValue> | DualKeyMap<TKeySource1, TKeySource2, TValue>;
    ...
    public clone(): CustomDualKeyMap<TKey, TKeySource1, TKeySource2, TValue> {
        const result = new CustomDualKeyMap<TKey, TKeySource1, TKeySource2, TValue>({ ...this.params, sourceMap: undefined });
    ...
    *[Symbol.iterator](): IterableIterator<readonly [TKey, TValue]> {
        for (const [dualKey, value] of this._dualKeyMap) {
            yield [this.params.createKey(dualKey), value] as const;
        }
    }
    ...
    public reduce<TResult>(reducer: (seed: TResult, element: TValue, key: TKey) => TResult, seed: TResult): TResult {
        let result = seed;
...
export const groupJoin = <TKey, TKeySource1, TKeySource2, TLeft, TRight>(left: ReadonlyCustomDualKeyMap<TKey, TKeySource1, TKeySource2, TLeft>, right: ReadonlyCustomDualKeyMap<TKey, TKeySource1, TKeySource2, TRight>): CustomDualKeyMap<TKey, TKeySource1, TKeySource2, GroupJoinResult<TLeft, TRight>> => {
