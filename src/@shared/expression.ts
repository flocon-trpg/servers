            ...
            case plain:
                switch (char) {
                    case '\\': {
                        ...
                        if (nextChar == null) {
                            return ResultModule.error({ message: '末尾を \\ にすることはできません。', index: cursor });
                        }
                        cursor++;
                    ...
                    case '}': {
                        return ResultModule.error({ message: '} に対応する { が見つかりません。', index: cursor });
                    }
                    default: {
            ...
            case expr2: {
                ...
                if (char === '}') {
                    switch (tail.reading.type) {
                        case 'Begin':
                            return ResultModule.error({ message: 'プロパティを空にすることはできません。', index: cursor });
                        case 'Bare':
                        case 'EndOfProp': {
                            if (tail.type === expr1) {
                                head.push({
                            ...
                            if (nextChar !== '}') {
                                return ResultModule.error({ message: '{{ を } で閉じることはできません。', index: cursor });
                            }
                            continue;
                        }
                        default:
                ...
                switch (tail.reading.type) {
                    case 'Begin': {
                        switch (char) {
                            ...
                            case '\'': {
                                tail = {
                                    ...tail,
                                    reading: { type: 'InSingleQuote', text: '' }
                                };
                                continue;
                            }
                            case '"': {
                                tail = {
                                    ...tail,
                                    reading: { type: 'InDoubleQuote', text: '' }
                                };
                                continue;
                            }
                            default: {
                                if (!bareKey.test(char)) {
                                    return ResultModule.error({ message: `${char} はこの場所で使うことはできません。`, index: cursor });
                                }
                                tail = {
                                    ...tail,
                                    reading: {
                    ...
                    case 'Bare': {
                        switch (char) {
                            ...
                            default: {
                                if (!bareKey.test(char)) {
                                    return ResultModule.error({ message: `${char} は ' か " で囲む必要があります。`, index: cursor });
                                }
                                tail = {
                                    ...tail,
                                    reading: {
                        ...
                        switch (char) {
                            ...
                            case '\\': {
                                ...
                                switch (nextChar) {
                                    ...
                                    case undefined:
                                        return ResultModule.error({ message: 'エスケープ文字の次に文字がありません。', index: cursor });
                                    default:
                                        // TOMLでは\uXXXXなどでunicodeを直接指定できるが、面倒なので今のところ実装は省略している。
                                        return ResultModule.error({ message: `\\${nextChar} は無効なエスケープシーケンスです。`, index: cursor });
                                }
                            }
                            default:
                                tail = {
