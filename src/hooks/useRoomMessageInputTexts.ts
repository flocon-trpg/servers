...
export type UseRoomMessageInputTextsResult = {
    ...
    setPublicMessageInputText: (
        ...
        key: PublicChannelKey.Without$System.PublicChannelKey
    ) => void;
...
export function useRoomMessageInputTexts({
    roomId,
}: {
    roomId: string;
}): UseRoomMessageInputTextsResult {
    const [publicMessageInputTexts, setPublicMessageInputTexts] = React.useState<
        ReadonlyMap<PublicChannelKey.Without$System.PublicChannelKey, string>
    >(new Map());
    const [privateMessageInputTexts, setPrivateMessageInputTexts] = React.useState<
    ...
    React.useEffect(() => {
        ...
        const subscriptions = [...subjects].map(([publicChannelKey, subject]) => {
            return subject.pipe(bufferTime(2000)).subscribe(ary => {
                ...
                writingMessageStatusMutation({
                    variables: { roomId, newStatus, publicChannelKey },
                });
            });
        });
    ...
    const setPublicMessageInputText = React.useCallback(
        (newMessage: string | undefined, key: PublicChannelKey.Without$System.PublicChannelKey) => {
            setPublicMessageInputTexts(oldMap =>
                produce(oldMap, draft => {
                    ...
                    if (oldMessage !== newMessage) {
                        subjects
                            .get(key)
                            ?.next({ prevMessage: oldMessage, currentMessage: newMessage });
                    }
                })
            );
        },
    ...
    const setPrivateMessageInputText = React.useCallback(
        (
            newMessage: string | undefined,
            visibleTo: ReadonlyArray<string> | ReadonlySet<string>
        ) => {
            setPrivateMessageInputTexts(oldMap =>
                produce(oldMap, draft => {
