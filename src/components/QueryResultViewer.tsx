...
const QueryResultViewer: React.FC<PropsWithChildren<Props>> = ({ children, error, loading, loadingTitle, compact }: PropsWithChildren<Props>) => {
    if (error != null) {
        if (compact) {
    ...
    if (loading === true) {
        if (compact) {
            return <span><Icon.LoadingOutlined />{loadingTitle ?? '読み込み中…'}</span>;
        }
