...
export const ActiveBoardSelectorModal: React.FC<Props> = ({ visible, onComplete }: Props) => {
    ...
    const options = React.useMemo(() => {
        ...
        return [...myBoards].sort(([, x], [, y]) => x.name.localeCompare(y.name)).map(([key, value]) => {
            return <Select.Option key={key} value={key}>{value.name}</Select.Option>;
        });
    }, [myBoards]);
