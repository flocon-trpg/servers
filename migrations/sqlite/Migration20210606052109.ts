...
export class Migration20210606052109 extends Migration {
  ...
  async up(): Promise<void> {
    this.addSql('create table `user` (`user_uid` varchar not null, `baas_type` text check (`baas_type` in (\'Firebase\')) not null, `is_entry` integer not null, primary key (`user_uid`));');
    ...
    this.addSql('create table `dice_piece_value_log` (`id` varchar not null, `character_created_by` varchar not null, `character_id` varchar not null, `created_at` datetime not null, `state_id` varchar not null, `value` json null, primary key (`id`));');
    this.addSql('create index `dice_piece_value_log_character_created_by_index` on `dice_piece_value_log` (`character_created_by`);');
    this.addSql('create index `dice_piece_value_log_character_id_index` on `dice_piece_value_log` (`character_id`);');
    this.addSql('create index `dice_piece_value_log_state_id_index` on `dice_piece_value_log` (`state_id`);');

    this.addSql('create table `number_piece_value_log` (`id` varchar not null, `character_created_by` varchar not null, `character_id` varchar not null, `created_at` datetime not null, `state_id` varchar not null, `value` json null, primary key (`id`));');
    this.addSql('create index `number_piece_value_log_character_created_by_index` on `number_piece_value_log` (`character_created_by`);');
    this.addSql('create index `number_piece_value_log_character_id_index` on `number_piece_value_log` (`character_id`);');
    this.addSql('create index `number_piece_value_log_state_id_index` on `number_piece_value_log` (`state_id`);');
    ...
    this.addSql('create index `dice_piece_value_log_room_id_index` on `dice_piece_value_log` (`room_id`);');

    this.addSql('alter table `number_piece_value_log` add column `room_id` varchar null;');
    this.addSql('create index `number_piece_value_log_room_id_index` on `number_piece_value_log` (`room_id`);');
