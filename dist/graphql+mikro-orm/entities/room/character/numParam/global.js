...
(function (GlobalNumParam) {
    ...
    })(MikroORM = GlobalNumParam.MikroORM || (GlobalNumParam.MikroORM = {}));
    let Global;
    (function (Global) {
        let ToGraphQL;
        (function (ToGraphQL) {
        ...
        Global.applyToEntity = async ({ em, parent, parentOp, operation, type, }) => {
            for (const [key, value] of operation) {
                let target;
                switch (type) {
                    case 'default': {
                        target = await em.findOne(mikro_orm_1.NumParam, { chara: { id: parent.id }, key });
                        if (target == null) {
                            target = new mikro_orm_1.NumParam({ key, isValuePrivate: false, chara: parent });
                            em.persist(target);
                        }
                        break;
                    }
                    case 'max': {
                        target = await em.findOne(mikro_orm_1.NumMaxParam, { chara: { id: parent.id }, key });
                        if (target == null) {
                            target = new mikro_orm_1.NumMaxParam({ key, isValuePrivate: false, chara: parent });
                            em.persist(target);
                        }
                        break;
                    }
                }
                const op = type === 'max' ? new mikro_orm_1.UpdateNumMaxParamOp({ key, updateCharaOp: parentOp }) : new mikro_orm_1.UpdateNumParamOp({ key, updateCharaOp: parentOp });
    ...
    })(Global = GlobalNumParam.Global || (GlobalNumParam.Global = {}));
    let GraphQL;
    (function (GraphQL) {
        let ToGlobal;
        (function (ToGlobal) {
    ...
    })(GraphQL = GlobalNumParam.GraphQL || (GlobalNumParam.GraphQL = {}));
    GlobalNumParam.transformerFactory = (createdByMe) => ({
        composeLoose: ({ first, second }) => {
            const valueProps = {
                isValuePrivate: Operations_1.ReplaceBooleanDownOperationModule.compose(first.isValuePrivate, second.isValuePrivate),
                value: Operations_1.ReplaceNullableNumberDownOperationModule.compose(first.value, second.value),
            };
        ...
        transform: ({ prevState, currentState, clientOperation, serverOperation }) => {
            ...
            if (createdByMe || !currentState.isValuePrivate) {
                twoWayOperation.value = Operations_1.ReplaceNullableNumberTwoWayOperationModule.transform({
...
})(GlobalNumParam = exports.GlobalNumParam || (exports.GlobalNumParam = {}));
