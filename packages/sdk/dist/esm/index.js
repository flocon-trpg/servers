import { simpleId, apply, roomTemplate, toOtError, clientTransform, diff, toUpOperation, parseState, parseUpOperation, stringifyUpOperation } from '@flocon-trpg/core';
...
const createRoomMessagesClient = ({ client, roomEventSubscription, }) => {
    const roomMessagesClient = new RoomMessagesClient();
...
class RoomStateManager {
    ...
    #onStateChangedLocally = new Subject();
    ...
    #subscribeOnStateChangedLocally({ client, clientId, }) {
        return this.#onStateChangedLocally
            .pipe(sampleTime(onChangedLocallySampleTime), map(() => {
...
const updateWritingMessageStatus = (client) => {
    ...
    const subscription = subject
        .pipe(bufferTime(bufferTimeValue), mergeMap(items => {
...
const createRoomClient = ({ client: clientSource, roomId, userUid, }) => {
    ...
    const roomJoinedSubscription = roomStateManager.stateStream
        .asObservable()
        .pipe(filter(x => x.type === 'joined'), take(1))
        .subscribe({
        next: () => {
...
const createTestRoomClientSource = () => {
    const roomMessageClient = new RoomMessagesClient();
