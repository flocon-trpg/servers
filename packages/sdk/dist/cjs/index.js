...
class StateManagerCore {
    ...
    onGet(operation, revisionTo, isByMyClient) {
        if (!Number.isInteger(revisionTo)) {
            utils.loggerRef.value.warn(`${revisionTo} is not an integer. onGet is cancelled.`);
            return;
        }
        if (revisionTo <= this._revision) {
            utils.loggerRef.value.info(`revisionTo of GetOperation is ${revisionTo}, but state revision is already ${this._revision}`);
            return;
        }
        if (this._pendingGetOperations.has(revisionTo)) {
            utils.loggerRef.value.warn(`stateManagerCore.__pendingGetOperations already contains ${revisionTo}`);
        }
...
class StateManager {
    ...
    onOtherClientsGet(operation, revisionTo) {
        ...
        utils.loggerRef.value.debug({ operation, revisionTo }, 'StateManager.onOtherClientGet');
    ...
    setUiState(state) {
        ...
        utils.loggerRef.value.debug({ state }, 'StateManager.setUiState');
    ...
    post() {
        ...
        utils.loggerRef.value.debug({ toPost }, 'StateManager.post begin');
        this._history?.beginPost(this, toPost);
        if (toPost === undefined) {
            utils.loggerRef.value.debug('StateManager.post is finished because toPost is undefined.');
            return undefined;
        }
        ...
        const onPosted = (onPosted) => {
            ...
            switch (onPosted.isSuccess) {
                case true:
                    if (onPosted.isId) {
                        utils.loggerRef.value.debug({ onPosted }, 'StateManager.post is completing as id');
                    ...
                    utils.loggerRef.value.debug({ onPosted }, 'StateManager.post is completing as non-id');
                    ...
                    return;
                case false:
                    utils.loggerRef.value.debug({ onPosted }, 'StateManager.post is completing as non-success');
                    this._history?.beforeEndPostAsNotSuccess(this);
                    this.core.cancelPost();
                    this._history?.afterEndPostAsNotSuccess(this);
                    return;
                case null:
                    utils.loggerRef.value.debug({ onPosted }, 'StateManager.post is completing as unknown result');
