...
class DeletableTree {
    #source;
    constructor(rootValue = Option.none()) {
    ...
    createSubTree(key, initValue) {
        ...
        const newTree = this.#source.createSubTree(key, absolutePath => Option.some(initValue(absolutePath)));
    ...
    get(key) {
        ...
        const result = this.#source.ensure(key, oldValue => Option.some(replacer(oldValue)), () => Option.none());
        const absolutePath = [];
        const ensure = () => {
    ...
    map(mapping) {
        const newTree = this.#source.map(oldValue => {
            ...
            return Option.some(mapping({ absolutePath: oldValue.absolutePath, value: oldValue.value.value }));
        });
        const result = new DeletableTree();
        result.#source = newTree;
        return result;
...
const parsePinoLogLevel = (source, envName) => {
    ...
    return Result.error(`${envName} value is invalid. Supported values: "fatal", "error", "warn", "info", "debug", "trace", "silent".`);
};
