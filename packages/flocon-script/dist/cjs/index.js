...
})(exports.FType || (exports.FType = {}));

const tryToPropertyName = (value) => {
    switch (value?.type) {
        case exports.FType.Number:
        case exports.FType.String:
            return value.raw.toString();
        case exports.FType.Symbol:
            return value.raw;
        default:
            return undefined;
    }
...
class JObjectCaster {
    ...
    static begin(source, astInfo) {
        return new JObjectCaster(source, {}, option.Option.none(), astInfo);
    }
    cast() {
    ...
    addArray() {
        if (this.source instanceof FArray) {
            return new JObjectCaster(this.source, { ...this.addedTypes, array: true }, option.Option.some(this.source), this.astInfo);
        }
        return this;
    }
    addBoolean() {
        if (this.source instanceof FBoolean) {
            return new JObjectCaster(this.source, { ...this.addedTypes, boolean: true }, option.Option.some(this.source.raw), this.astInfo);
        }
        return this;
    }
    addFunction() {
        if (this.source instanceof FFunction) {
            ...
            return new JObjectCaster(source, { ...this.addedTypes, function: true }, option.Option.some((isNew) => (args) => source.exec({ args, isNew, astInfo: this.astInfo })), this.astInfo);
        }
        return this;
    }
    addNull() {
        if (this.source === null) {
            const source = this.source;
            return new JObjectCaster(source, { ...this.addedTypes, null: true }, option.Option.some(null), this.astInfo);
        }
        return this;
    }
    addNumber() {
        if (this.source instanceof FNumber) {
            return new JObjectCaster(this.source, { ...this.addedTypes, number: true }, option.Option.some(this.source.raw), this.astInfo);
        }
        return this;
    }
    addObject() {
        if (this.source instanceof FObject) {
            return new JObjectCaster(this.source, { ...this.addedTypes, object: true }, option.Option.some(this.source), this.astInfo);
        }
        return this;
    }
    addString() {
        if (this.source instanceof FString) {
            return new JObjectCaster(this.source, { ...this.addedTypes, string: true }, option.Option.some(this.source.raw), this.astInfo);
        }
        return this;
    }
    addSymbol() {
        if (this.source instanceof FSymbol) {
            return new JObjectCaster(this.source, { ...this.addedTypes, symbol: true }, option.Option.some(this.source.raw), this.astInfo);
        }
        return this;
    }
    addUndefined() {
        if (this.source === undefined) {
            const source = this.source;
            return new JObjectCaster(source, { ...this.addedTypes, undefined: true }, option.Option.some(undefined), this.astInfo);
        }
        return this;
    }
}
...
const toProgram = (script) => {
    ...
    return acorn.parse(script, { ecmaVersion: 2020, ranges: true });
};
...
class FArrayClass extends FFunction {
    ...
    onGetting({ key, astInfo }) {
        switch (key) {
            case 'isArray': {
                return option.Option.some(new FFunction(({ args, isNew }) => {
                    FArrayClass.prepareStaticMethod(isNew, astInfo);
                    const arg = args[0];
                    return new FBoolean(arg?.type === exports.FType.Array);
                }));
            }
            default:
                return option.Option.none();
...
class FSymbolClass extends FFunction {
    ...
    onGetting({ key }) {
        switch (key) {
            case 'iterator': {
                return option.Option.some(new FSymbol(Symbol.iterator));
            }
            default:
                return option.Option.none();
        }
...
exports.FRecord = FRecord;
exports.FRecordRef = FRecordRef;
exports.FString = FString;
exports.FSymbol = FSymbol;
exports.FTypedArray = FTypedArray;
exports.ScriptError = ScriptError;
exports.arrayClass = arrayClass;
exports.beginCast = beginCast;
exports.createConsoleClass = createConsoleClass;
exports.exec = exec;
exports.mapClass = mapClass;
exports.symbolClass = symbolClass;
exports.test = test;
