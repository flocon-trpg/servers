...
export const toDownOperation = (source: TwoWayOperation): DownOperation => {
    return {
        ...
        memo: undefined,
        name: undefined,
        ...BoardPositionBase.toDownOperation(source),
    };
};
...
export const toUpOperation = (source: TwoWayOperation): UpOperation => {
    return {
        ...
        memo: undefined,
        name: undefined,
        ...BoardPositionBase.toUpOperation(source),
    };
};

export const apply: Apply<State, UpOperation> = ({ state, operation }) => {
    const boardPosition = BoardPositionBase.apply({ state, operation });
...
export const applyBack: Apply<State, DownOperation> = ({ state, operation }) => {
    const boardPosition = BoardPositionBase.applyBack({ state, operation });
    if (boardPosition.isError) {
        return boardPosition;
    }
    const result = { ...state };
...
export const restore: Restore<State, DownOperation, TwoWayOperation> = ({
    ...
    const boardPosition = BoardPositionBase.restore({ nextState, downOperation });
    if (boardPosition.isError) {
        return boardPosition;
    }
...
export const diff: Diff<State, TwoWayOperation> = ({ prevState, nextState }) => {
    const resultType: TwoWayOperation = {
        ...BoardPositionBase.diff({ prevState, nextState }),
        $v: 2,
        $r: 1,
    };
...
export const clientTransform: ClientTransform<UpOperation> = ({ first, second }) => {
    const boardPosition = BoardPositionBase.clientTransform({ first, second });
